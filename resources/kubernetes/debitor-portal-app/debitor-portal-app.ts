...
export const debitorPortalApp = new DeploymentComponent(
	'debitor-portal-app',
	{
		...
		envFrom: [
			{
				secretRef: {
					name: debitorPortalCredentials.metadata
						.name,
				},
			},
			{
				secretRef: {
					name: debitorPortalRestApiCredentials
						.metadata.name,
				},
			},
			{
				configMapRef: {
					name: debitorPaymentProvider.metadata
						.name,
				},
			},
		],
		env: [
			{
				...
				value: customers.apply(customers =>
					JSON.stringify(customers.filter(customer =>
